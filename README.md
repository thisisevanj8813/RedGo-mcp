...
```json
{
  "mcpServers": {
    "redgo": {
      "command": "uv",
      "args": ["run", "--directory", "/path/to/RedGo-mcp", "redgo-mcp"]
    }
  }
}
```

贴进 Claude Code 的 `.mcp.json`（Codex / Cursor 同理），agent 立刻多出 4 个小红书工具。发布到 PyPI 后更短：`"command": "uvx", "args": ["redgo-mcp"]`。
...
**1. 下载并安装**（需要 [uv](https://docs.astral.sh/uv/)）
...
```bash
uv run redgo launch-chrome
```

运行后会打开一个浏览器窗口（若已有登录好的实例则直接复用、不重复弹窗，属正常）。**提示用户在此窗口扫码登录小红书**，登录态长期保存，窗口保持开启（最小化即可）。

**3. 写入 MCP 配置**：把下面这段加进用户 AI 工具的配置文件（如 Claude Code 的 `.mcp.json`），`--directory` 路径改为上一步的克隆目录。Cursor、Codex 同理。
...
**4. 提醒用户**：配置写入后需**新开一个对话**（MCP 工具在会话启动时加载），RedGo 的四个工具才会生效。

> 排查：运行 `uv run redgo status` 可检查浏览器连接与登录状态。
...
1. **下载**：`git clone https://github.com/thisisevanj8813/RedGo-mcp && cd RedGo-mcp` 然后 `uv sync`（需先装 [uv](https://docs.astral.sh/uv/)）
2. **登录**：`uv run redgo launch-chrome`，在弹出的窗口里扫码登录小红书（非主力小号），窗口别关
3. **接入**：把上面那段 `.mcp.json` 配置贴进你的 AI 工具，路径改成你的克隆目录，然后**新开一个对话**

随时运行 `uv run redgo status` 看连接和登录状态。
